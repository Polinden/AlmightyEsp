...
Есть и обещанная возможность участвовать в получении MQTT сообщений от брокера для абсолютной интеграции у систему инетрент вещей. Данная функция пока что предполагает указание ip MQTT-сервера в коде, но в следующей версии будет предоставлена возможность настройки через WiFiManager.
