...
- при старте нового устройства одновременно с конфигурацией подключения к WiFi Вы можете указать ip адрес MQTT сервера
...
- реле подписывается на тему "erelay2" и реагирует на сообщения в формате "n=on" / "n=off" (где n-номер реле) включаясь и выключаясь
- названия тем, как и тонкие настройки соединения находятся в файле "settings.h" 
...
- если же нужна перепрошивка ПО, то оно выполняется "по воздуху" без отключения устройства
