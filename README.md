...
Есть и обещанная возможность участвовать в информировании других IoT-устройств через брокера MQTT (https://mosquitto.org) для абсолютной интеграции в систему инернета вещей. Данная функция пока что предполагает настройку ip прямо в коде, но в следующей версии будет предоставлена возможность настройки опций MQTT через WiFiManager. Для ее активации раскомментируйте строку <i>#include "MQTT.h"</i> (файл Pattern_ESP.ino) и Ваше реле начнет отправлять сообщения MQTT брокеру об изменении своего статуса ("вкл"/"выкл"). Кастомизируйте под свои потребности.
